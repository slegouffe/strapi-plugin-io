...
class SocketIO {
  constructor(options) {
    this._socket = new socket_io.Server(strapi.server.httpServer, options);
...
const strategies = ({ strapi: strapi2 }) => {
  ...
  const role = {
    ...
    verify: function(auth, config2) {
      ...
      const isAllowed = fp.pipe(
        fp.castArray,
        fp.every((scope) => ability.can(scope))
      )(config2.scope);
      if (!isAllowed) {
  ...
  const token = {
    ...
    authenticate: async function(auth) {
      ...
      if (!apiToken.lastUsedAt || dateFns.differenceInHours(currentDate, dateFns.parseISO(apiToken.lastUsedAt)) >= 1) {
